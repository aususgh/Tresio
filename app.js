...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (savedTheme === 'light') {
        ...
        // Por defecto entra en modo oscuro
...
function updateLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
        (position) => {
            ...
            // Guardar para uso local/offline futuro
...
function displayMap(lat, lon, timestamp) {
    ...
    // Iframe nativo apuntando a Google Maps
...
// ==========================================
// FUNCIÓN PARA EL TIEMPO RELATIVO (NUEVO)
// ==========================================
...
function loadPhrases() {
    ...
    phrases.forEach(phrase => {
        ...
        // Renderización condicional del timestamp
        let timeHtml = '';
        if (phrase.timestamp) {
            const timeStr = obtenerTiempoRelativo(phrase.timestamp);
            // Color gris claro/transparente para tus mensajes (para que contraste en fondo azul) y gris estándar para los ajenos.
...
function handleAddPhrase(e) {
    ...
    if (text) {
        ...
        // Se inyecta el timestamp al guardar el mensaje
        phrases.push({ 
            text: text, 
            author: currentUser,
